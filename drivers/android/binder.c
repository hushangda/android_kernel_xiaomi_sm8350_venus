			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		/*
		 * Let the caller know when an async transaction reaches a frozen
		 * process and is queued until the target process is unfrozen.
		 */
...
static int binder_request_freeze_notification(
...
static int binder_clear_freeze_notification(
		struct binder_proc *proc, struct binder_thread *thread,
		struct binder_handle_cookie *handle_cookie)
{
	struct binder_ref_freeze *freeze;
	...
	binder_node_lock(ref->node);
	if (!ref->freeze) {
		binder_user_error("%d:%d BC_CLEAR_FREEZE_NOTIFICATION not active\n",
				  proc->pid, thread->pid);
		binder_node_unlock(ref->node);
		binder_proc_unlock(proc);
		return -EINVAL;
...
static int binder_freeze_notification_done(struct binder_proc *proc,
		struct binder_thread *thread, binder_uintptr_t cookie)
{
	struct binder_ref_freeze *freeze = NULL;
	struct binder_work *w;

	binder_inner_proc_lock(proc);
	list_for_each_entry(w, binder_get_delivered_freeze(proc), entry) {
		struct binder_ref_freeze *candidate;

		candidate = container_of(w, struct binder_ref_freeze, work);
		if (candidate->cookie == cookie) {
			freeze = candidate;
			break;
		}
	}
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error.cmd == BR_OK) {
		...
		case BC_REQUEST_FREEZE_NOTIFICATION: {
			...
			if (copy_from_user(&handle_cookie, ptr,
	...
	while (1) {
		...
		case BINDER_WORK_FROZEN_BINDER: {
			...
			binder_enqueue_work_ilocked(w,
					binder_get_delivered_freeze(proc));
			binder_inner_proc_unlock(proc);

			if (put_user(BR_FROZEN_BINDER,
				     (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	INIT_LIST_HEAD(binder_get_delivered_freeze(proc));
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	binder_release_work(proc, binder_get_delivered_freeze(proc));
...
static int __init binder_create_pools(void)
{
	...
	/* binder_open() allocates binder_proc_ext, not the embedded binder_proc. */
	binder_proc_pool = KMEM_CACHE(binder_proc_ext, SLAB_HWCACHE_ALIGN);
	if (!binder_proc_pool)
		goto err_proc_pool;
