...
void test_openat2_flags(void)
{
	...
	for (int i = 0; i < ARRAY_LEN(tests); i++) {
		...
		path = (test->how.flags & O_CREAT) ? "ksft.openat2_tmpfile" : ".";
		unlink(path);
		...
		if (fd >= 0) {
			...
			if (!(test->how.flags & O_LARGEFILE))
				fdflags &= ~O_LARGEFILE;
			failed |= (fdflags != (unsigned int)test->how.flags);
		}
...
int main(int argc, char **argv)
{
	...
	int oldcwd = open(".", O_RDONLY | O_DIRECTORY);

	if (oldcwd < 0)
		ksft_exit_fail_msg("cannot save cwd\n");
	snprintf(dirname, sizeof(dirname), "%s/ksft-openat2.XXXXXX",
		 tmpdir ? tmpdir : "/tmp");
	if (!mkdtemp(dirname) || chdir(dirname))
		ksft_exit_fail_msg("cannot create scratch directory\n");
	ksft_print_header();
