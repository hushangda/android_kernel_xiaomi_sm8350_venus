...
int setup_testdir(void)
{
	...
	snprintf(scratch, sizeof(scratch), "%s/ksft-openat2-rename.XXXXXX",
		 tmpdir ? tmpdir : "/tmp");

	/* Make the top-level directory. */
	if (!mkdtemp(scratch))
		ksft_exit_fail_msg("setup_testdir: failed to create tmpdir\n");
	dfd = open(scratch, O_PATH | O_DIRECTORY);
	if (dfd < 0)
...
pid_t spawn_attack(int dirfd, char *a, char *b)
{
	...
	/* Swap @a and @b. */
	for (;;)
		sys_renameat2(dirfd, a, dirfd, b, RENAME_EXCHANGE);
	exit(1);
}
...
void test_rename_attack(int resolve)
{
	...
	if (!openat2_supported) {
		ksft_test_result_skip("openat2 unavailable; cannot test scoped lookup\n");
		return;
	}
	...
	if (child < 0)
		ksft_exit_fail_msg("cannot fork rename worker\n");
	...
	close(afd);
	unlinkat(dfd, "a/c", AT_REMOVEDIR);
	unlinkat(dfd, "a", AT_REMOVEDIR);
	unlinkat(dfd, "b", AT_REMOVEDIR);
	close(dfd);
	rmdir(scratch);
}
